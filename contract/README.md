...
**Presentación patrocinada: el ciudadano no paga DUST.** El único gasto recurrente del
sistema caía sobre el holder, y justo en el momento en que un policía le pide los
papeles — quedarse sin fondos equivalía a no tener el certificado. Se resuelve con el
Capacity Exchange de SundaeSwap: `sponsoredTransactionsWalletProvider` reemplaza el
`walletProvider` y manda la transacción ya probada a un server que le pone el DUST. No
cambia el contrato, sólo quién paga. De yapa mejora la privacidad: con una wallet por
ciudadano, todas sus presentaciones quedaban ligadas entre sí por su dirección; con un
patrocinador único salen todas de la misma y se vuelven un conjunto de anonimato.
